...
(function(){
  ...
  function placePlusAtCaret(range){
    ...
    const isTextBlock = tag === 'p' || tag === 'div' || tag === 'h1' || tag === 'h2' || tag === 'h3' || tag === 'blockquote';
    if (!isTextBlock) { plus.style.display='none'; plus.classList.remove('open'); return; }
    if ((block.textContent || '').trim() !== '') { plus.style.display='none'; plus.classList.remove('open'); return; }
