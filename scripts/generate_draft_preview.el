...
(function(){
  ...
  function onBodyArrowKey(e){
    ...
    const range = sel.getRangeAt(0);
    if (e.key === 'ArrowDown'){
      if (isVoidElement(block) || isAtEndOfBlock(range, block)){
        e.preventDefault();
      ...
      if (isVoidElement(block) || isAtStartOfBlock(range, block)){
        e.preventDefault();
  ...
  plusMenu.addEventListener('click', async (e)=>{
    ...
    if (action === 'ditaa') {
      ...
      if (pre) { pre.textContent = '+--------+   +--------+\\n|  Node  |-->|  Node  |\\n+--------+   +--------+'; pre.dataset.raw = pre.textContent; toCodeHighlightMode(pre); }
      return;
    }
    if (action === 'plantuml') {
      insertCodeBlockAtCursor('plantuml');
      const pre = body.querySelector('pre.code-block[data-lang=\"plantuml\"]:last-of-type');
      if (pre) { pre.textContent = '@startuml\\nAlice -> Bob: Hello\\nBob --> Alice: Hi\\n@enduml'; pre.dataset.raw = pre.textContent; toCodeHighlightMode(pre); }
      return;
    }
    if (action === 'ascii') {
      insertCodeBlockAtCursor('text');
      const pre = body.querySelector('pre.code-block[data-lang=\"text\"]:last-of-type');
      if (pre) { pre.textContent = '    /\\\\\\n   /  \\\\\\n  /----\\\\\\n /      \\\\\\n/________\\\\'; pre.dataset.raw = pre.textContent; toCodeHighlightMode(pre); }
      return;
    }
    if (action === 'table') {
      insertTableAtCursor();
