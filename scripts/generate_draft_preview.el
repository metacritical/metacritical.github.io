...
<div class=\"draft-tools\" id=\"draft-tools\">
  ...
  <button type=\"button\" data-action=\"text-color\">Text</button>
  <button type=\"button\" data-action=\"code-lang\">Lang</button>
...
(function(){
  ...
  tools.addEventListener('click', async (e)=>{
    ...
    if (action === 'text-color') {
      const color = (foregroundCustom && foregroundCustom.value) || '#1d1b18';
    ...
    if (action === 'code-lang') {
      ...
      const pre = range ? nearestPre(range.commonAncestorContainer) : null;
      if (!pre) return;
      const current = (pre.dataset.lang || pre.getAttribute('data-lang') || 'text').trim();
      const lang = (await askInput({ title:'Code Language', label:'Language', placeholder:'python, sh, js', value: current || 'text', okText:'Apply' }) || current).trim();
      if (!lang) return;
      pre.dataset.lang = lang;
      pre.setAttribute('data-lang', lang);
      pre.classList.add('code-block', 'src', 'src-' + lang);
      markDirty();
      toCodeHighlightMode(pre);
    }
  ...
  plusMenu.addEventListener('click', async (e)=>{
    ...
    const action = btn.dataset.action;
    if (action === 'image') {
      const url = await askInput({ title:'Insert Image', label:'Image URL', placeholder:'https://.../image.png', okText:'Insert' });
      if (url) insertHtmlAtCursor('<figure class=\"image-block\" style=\"width:760px;max-width:100%%;\"><img src=\"' + url + '\" alt=\"\"><figcaption></figcaption></figure><p><br></p>');
      return;
    }
    ...
    (when (file-directory-p posts-dir)
      (dolist (post (sort (directory-files posts-dir t "\\.org\\'") #'string<))
        (let* ((parsed (sds/parse-org post))
               (title (plist-get parsed :title))
               (slug (sds/slugify (file-name-base post)))
               (date (plist-get parsed :date))
               (tags (or (plist-get parsed :tags) '("untagged")))
               (body (plist-get parsed :body))
               (html-body (plist-get parsed :html-body))
